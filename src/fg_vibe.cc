...
const struct FGVIBEParams kDefaultVibeParams = {1, 20, 4, 17, 2, 16};

// FGVIBE::FGVIBE(int channels, int samples, int pixel_neighbor,
...
FGVIBE::FGVIBE(void *params) {
  ...
  if (params != NULL) {
    params_ = *(static_cast<FGVIBEParams *>(params));
  } else {
    params_ = kDefaultVibeParams;
  }
  channels_ = params_.channels;
  samples_ = params_.samples;
  pixel_neighbor_ = params_.pixel_neighbor;
  AddParam("distance threshold", &params_.distance_threshold);
  AddParam("matching threshold", &params_.matching_threshold);
  AddParam("update factor", &params_.update_factor);
  SetModuleName("VIBE");
}
...
void FGVIBE::Process(IplImage *pImg) {
  cv::Mat img(pImg);
  cv::Mat gray;
  if(img.channels() == 3)
...
cv::Vec2i FGVIBE::getRndNeighbor(int i, int j) {
...
void FGVIBE::init(const cv::Mat &img) {
...
void FGVIBE::update(const cv::Mat &img) {
  ...
  for (int i = 0; i < img.rows; i++) {
    for (int j = 0; j < img.cols; j++) {
      ...
      for (int s = 0; s < samples_; s++) {
        ...
        if (matching_counter > params_.matching_threshold) {
          flag = true;
          break;
        }
      }
